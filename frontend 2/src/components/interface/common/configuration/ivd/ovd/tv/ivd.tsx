...
import { IvdOvdTvPost, DeleteDevices, fetchDevicesDetails, updateDeviceName, updatePortName, fetchDevices } from "../../../../../../../redux/actions/interface/interface";
...
export const IVD = (props: any) => {
  const { platformNumbers } = props;
  const dispatch = useDispatch();
  const { currentSelectedCdsEthernetDevice, currentSelectedCdsPortNumber, cdsData, ivdState, ivdMessage, deleteState, formState } = useSelector(interfaceStateSelector)
  ...
  useEffect(() => {
    if (cdsData && cdsData.children) {
      ...
      // if(detail && detail.deviceType ==="ivd" && detail.detail && currentSelectedCdsPortNumber === detail.detail.portNumber){
  ...
  //                 Select the IVD type and add IP address to configure
  //               </Typography>
  ...
  //             </Grid>
  //             {appUser.userRole == "ROLE_STATION MASTER" ? <></> :
  //             <div style={{ paddingTop: "2.57%", display: "flex", justifyContent: "center", alignItems: "center", width: "100%" }}>
      ...
      <Grid item xs={12} style={{ height: "100%" }}>
        <Card style={{ padding: "30px", height: "100%" }} className={appUser.userRole == "ROLE_STATION MASTER" ? "overlapBackground ivdCard" : "ivdCard"}>
          ...
          <Grid container style={{ paddingTop: "3.12%" }} className="mb-17">
            <Grid item className="align-P" xs={4} md={6} lg={6} xl={5}>
              <Typography className="ivdSubItem">
                IVD IP Address<span className="asterisk">*</span>
              </Typography>
            </Grid>
          ...
          {appUser.userRole == "ROLE_STATION MASTER" ? <></> :
            <Grid container spacing={2} style={{ paddingTop: "1.57%", display: "flex", justifyContent: "center", alignItems: "center", width: "100%" }}>
