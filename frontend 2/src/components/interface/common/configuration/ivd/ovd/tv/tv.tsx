...
export const TV = (props: any) => {
  ...
  const { currentSelectedCdsEthernetDevice, currentSelectedCdsPortNumber, cdsData, tvState, tvMessage, deleteState, formState } = useSelector(interfaceStateSelector)
  ...
  useEffect(() => {
    if (formState.isSuccess == true) {
      if (cdsData && cdsData.children) {
        let detail: any = cdsData.children.find((port: any) => port.portNumber === currentSelectedCdsPortNumber);
        if (detail && detail.deviceType === "tvDisplay" && detail.detail && currentSelectedCdsPortNumber === detail.detail.portNumber) {
  ...
  //             Select the TV type and add IP address to configure
  //           </Typography>
  //           <Grid container style={{ paddingTop: "2.12%" }}>
  ...
  //               Cancel
  //             </Button>
  //             {interfaceInput.id !== 0 ? <><Button onClick={handleDelete} variant="outlined" className="ivdDeleteBtn">
      ...
      <Card style={{ padding: "30px", height: "100%" }} className={appUser.userRole == "ROLE_STATION MASTER" ? "overlapBackground ivdCard" : "ivdCard"}>
        ...
        <Grid container className="mb-17">
          <Grid item xs={4} md={6} lg={6} xl={5} className="ivdSubItem">
            <Typography>
              TV IP Address<span className="asterisk">*</span>
            </Typography>
          </Grid>
        ...
        {appUser.userRole == "ROLE_STATION MASTER" ? <></> :
          <Grid container spacing={2} style={{ paddingTop: "2.57%", display: "flex", justifyContent: "center", alignItems: "center", width: "100%" }}>
